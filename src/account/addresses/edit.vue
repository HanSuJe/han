  ...
  <div >
    ...
    <section class="dx_body  ">
      <form class="mui-input-group">
        ...
        <div class="mui-input-row fz14 pr bgff mt10" @click="select()">
          <label>设为默认</label>
          <span class="f_b sd_gxsd yj cz" :class="{act:isMoren}">
...
export default {
  ...
  methods: {
    select() {
      if (this.form_s.is_default) {
        this.form_s.is_default = false;
        this.isMoren = false;
      } else {
        this.form_s.is_default = true;
        this.isMoren = true;
      }
    },
    sanchu(uid) {
  ...
  mounted() {
    ...
    if (this.$route.query.province) {
      ...
      this.form_s.diqu =
        this.$route.query.province + "-" + this.$route.query.city;
    }
  }
};
</script>
